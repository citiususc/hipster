...
public class ScaleWeightedNodeFactory<A,S,C extends Comparable<C>> implements NodeFactory<A,S, WeightedNode<A,S,C>>{
    ...
    public WeightedNode<A, S, C> makeNode(WeightedNode<A, S, C> fromNode, Transition<A, S> transition) {
        ...
        estimation = this.heuristicFunction.estimate(transition.getState());
        score = costAccumulator.apply(cost, scalarOperation.scale(estimation, scaleFactor));

        return new WeightedNode<A,S,C>(fromNode, transition.getState(), transition.getAction(), cost, estimation, score);
    }
